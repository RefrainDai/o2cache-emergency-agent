...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(description="Evaluate QwenVL 8B origin model on valid set.")
	parser.add_argument(
		"--dataset-file",
		type=str,
	...
	parser.add_argument(
		"--output-json",
		type=str,
		default="/root/autodl-tmp/Outputs/qwen_valid_eval_8b_origin.json",
	)
	...
	for start in range(0, total_samples, batch_size):
		...
		for offset, (sample, raw_pred, allowed_space) in enumerate(zip(chunk_samples, chunk_preds, chunk_answer_spaces), start=1):
			...
			if idx % 50 == 0 or idx == total_samples:
				print(f"[Qwen8B-origin] progress: {idx}/{total_samples}")
